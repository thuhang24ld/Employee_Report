"""
iPos Fabi - Auto Crawler (Sale change log)
=======================================
...
Chạy: python crawl_sale_change_log.py
"""
# -*- coding: utf-8 -*-
...
STORE_UID      = "b22c4539-4eb7-4c2e-ab61-129e89916566"
LIST_STORE_UID = "b22c4539-4eb7-4c2e-ab61-129e89916566,291963e2-371c-45f8-aace-694b5f7d9a5b,d966f1d9-e34b-4d25-bc9d-258e98cf654d"
ACCESS_TOKEN   = "5c885b2ef8c34fb7b1d1fad11eef7bec"

OUTPUT_DIR       = r"D:\BanRau_IPos_Data\sale_change_log"
...
DETAIL_URL = "https://posapi.ipos.vn/api/v3/pos-cms/sale-change-log"
...
def crawl_one_day(d, token):
    ...
    resp = requests.get(
        ...
        params=params,
        timeout=120  # tăng timeout vì data nhiều
    )
 
    if resp.status_code == 401:
        log.warning("Token hết hạn, login lại...")
        token = login()
        return crawl_one_day(d, token)
    ...
    log.info(f"   ✅ {len(records)} records từ {len(LIST_STORE_UID.split(','))} cửa hàng")
    return records
...
def export_excel(records, d):
    filename = os.path.join(OUTPUT_DIR, f"sale_change_log_{d}.xlsx")
    if not records:
...
# ============================================================
# GMAIL NOTIFICATION (dung SMTP voi Gmail App Password)
# ============================================================
 
GMAIL_FROM     = "thuhang24.ld@gmail.com"
GMAIL_PASSWORD = "wjqi vuyb kixb qtnr"   # <-- App Password 16 ky tu tu Google
GMAIL_TO       = "thuhang24.ld@gmail.com"
def send_email(subject, body, is_error=False):
    try:
        ...
        log.info(f"Da gui email: {subject}")
    except Exception as e:
        log.warning(f"Khong gui duoc email: {e}")
