"""
iPos Fabi - Crawl Sale By Date (tất cả cửa hàng)
=================================================
Cài: pip install requests pandas openpyxl
Chạy: python crawl_sale_by_date.py
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(OUTPUT_DIR, "crawler_hoadon.log"), encoding="utf-8"),
    ]
)
...
def crawl_store_one_day(d, store_uid, token):
    """Crawl sale-by-date của 1 store, 1 ngày (có phân trang)"""
    records  = []
    page     = 1
    ...
    while True:
        ...
        if not resp.ok:
            log.warning(f"   Store {store_uid[:8]}... HTTP {resp.status_code}")
            break
        ...
        for r in batch:
            r["_date"]      = str(d)
            r["_store_uid"] = store_uid

        records.extend(batch)

        total = data.get("total") or (data.get("meta") or {}).get("total") or 0
...
def crawl_all_stores(d, token):
    """Crawl tất cả stores cho 1 ngày"""
    all_records = []
    for i, store_uid in enumerate(STORE_UIDS):
        records = crawl_store_one_day(d, store_uid, token)
        log.info(f"   Store {i+1}/{len(STORE_UIDS)} ({store_uid[:8]}...): {len(records)} records")
...
def export_excel(records, d):
    filename = os.path.join(OUTPUT_DIR, f"hoa_don_{d}.xlsx")
    if not records:
...
if __name__ == "__main__":
    ...
    log.info(f"🚀 Crawl sale-by-date ngày {target} ({len(STORE_UIDS)} cửa hàng)")
    try:
        if not wait_for_network():
            sys.exit(1)
        token   = get_token()
        records = crawl_all_stores(target, token)
        log.info(f"📊 Tổng: {len(records)} records")
